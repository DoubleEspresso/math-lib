...
namespace Math {
  namespace Matrix {
    ...
    template<typename T>
    void axb_gpu(const T * a, const T * b, T * c, const int ra, const int ca, const int cb) {
      mm_device<T>(a, b, c, ra, ca, cb);
    }    
  };
};
