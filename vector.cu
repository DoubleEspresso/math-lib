...
template<typename T> __global__ void d_adotb(const T * a, const T * b, T * out, const int N);
...
template __global__ void d_adotb(const char * a, const char * b, char * out, const int N);
template __global__ void d_adotb(const int * a, const int * b, int * out, const int N);
template __global__ void d_adotb(const float * a, const float * b, float * out, const int N);
template __global__ void d_adotb(const double * a, const double * b, double * out, const int N);

namespace Math
{
	template<typename T>
	void Vector::adotb_gpu(const T * a, const T * b, T * c, const int dim)
	{
		...
		*c = 0;
		...
		d_adotb<T> << <blocks, threads, threads * sizeof(T) >> >(ad, bd, cd, dim);
...
template<typename T> __global__ void d_adotb(T * a, T * b, T * out, int N)
{
